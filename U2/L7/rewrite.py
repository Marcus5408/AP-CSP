...
class Wishlister(tk.Tk):
    ...
    def __init__(self):
        ...
        self.shuffle_button = ttk.Button(
            text="Shuffle Games", command=self.shuffle_items_ui
        )
        self.find_max_button = ttk.Button(
            text="Find Max. Price", command=self.find_max_ui
        )
        ...
        self.title_label.grid(
            row=0, column=0, columnspan=6, padx=5, pady=5, sticky="nsew"
        )
        self.add_button.grid(row=1, column=0, padx=5, pady=(0, 5), sticky="nsew")
        self.remove_button.grid(
            row=1, column=1, padx=(0, 5), pady=(0, 5), sticky="nsew"
        )
        self.insert_button.grid(
            row=1, column=2, padx=(0, 5), pady=(0, 5), sticky="nsew"
        )
        self.swap_button.grid(
            row=1, column=3, padx=(0, 5), pady=(0, 5), sticky="nsew"
        )
        self.shuffle_button.grid(
            row=1, column=4, padx=(0, 5), pady=(0, 5), sticky="nsew"
        )
        self.find_max_button.grid(
            row=1, column=5, padx=(0, 5), pady=(0, 5), sticky="nsew"
        )
        self.wishlist_display.grid(
            row=2, column=0, columnspan=6, padx=5, pady=(0, 5), sticky="nsew"
        )
    ...
    def add_item_ui(self):
        ...
        add_item_dialog.game_link_label = ttk.Label(
            add_item_dialog, text="Game Link:", width=11
        )
        add_item_dialog.game_link_entry = ttk.Entry(add_item_dialog, width=50)
        # this button does basically everything. it gets the game's ID, name,
        # and price from the steam API and adds it to the database.
        add_item_dialog.add_button = ttk.Button(
            ...
            command=lambda: (
                self.change_database(
                    action="add",
                    game_id=self.get_steam_details(
                        "id", add_item_dialog.game_link_entry.get()
                    ),
                    data={
                        "game_id": self.get_steam_details(
                            "id", add_item_dialog.game_link_entry.get()
                        ),
                        "name": self.get_steam_details(
                            "name", add_item_dialog.game_link_entry.get()
                        ),
                        "price": self.get_steam_details(
                            "price", add_item_dialog.game_link_entry.get()
                        ),
        ...
        add_item_dialog.cancel_button = ttk.Button(
            add_item_dialog, text="Cancel", command=add_item_dialog.destroy
        )

        # Add the widgets to the dialog
        add_item_dialog.title_label.grid(
            row=0, column=0, columnspan=2, padx=5, pady=5, sticky="nsew"
        )
        add_item_dialog.game_link_label.grid(
            row=1, column=0, padx=5, pady=5, sticky="nsew"
        )
        add_item_dialog.game_link_entry.grid(
        ...
        add_item_dialog.cancel_button.grid(
    ...
    def remove_item_ui(self):
        ...
        remove_item_dialog.title_label = ttk.Label(
            remove_item_dialog, text="Remove Game"
        )
        ...
        cancel_button = ttk.Button(
            remove_item_dialog, text="Cancel", command=remove_item_dialog.destroy
        )

        # Add the widgets to the dialog

        remove_item_dialog.title_label.grid(
            row=0, column=0, columnspan=2, padx=5, pady=5, sticky="nsew"
        )
    ...
    def insert_item_ui(self):
        ...
        insert_item_dialog.title_label = ttk.Label(
            insert_item_dialog, text="Insert Game"
        )

        insert_item_dialog.title_label.config(font=("Arial", 12))

        insert_item_dialog.game_link_label = ttk.Label(
            insert_item_dialog, text="Game Link:", width=11
        )

        insert_item_dialog.game_link_entry = ttk.Entry(insert_item_dialog, width=50)

        insert_item_dialog.game_position_label = ttk.Label(
            insert_item_dialog, text="Position:", width=11
        )

        insert_item_dialog.game_position_spinbox = tk.Spinbox(
            insert_item_dialog, from_=1, to=len(self.database) + 1, width=48
        )
        ...
        ok_button = ttk.Button(
            ...
            command=lambda: (
                self.change_database(
                    action="insert",
                    game_id=self.get_steam_details(
                        "id", insert_item_dialog.game_link_entry.get()
                    ...
                    data={
                        ...
                        "name": self.get_steam_details(
                            "name", insert_item_dialog.game_link_entry.get()
                        ),
                        "price": self.get_steam_details(
                            "price", insert_item_dialog.game_link_entry.get()
                        ),
        ...
        cancel_button = ttk.Button(
            insert_item_dialog, text="Cancel", command=insert_item_dialog.destroy
        )

        # Add the widgets to the dialog

        insert_item_dialog.title_label.grid(
            row=0, column=0, columnspan=2, padx=5, pady=5, sticky="nsew"
        )

        insert_item_dialog.game_link_label.grid(
            row=1, column=0, padx=5, pady=5, sticky="nsew"
        )

        insert_item_dialog.game_link_entry.grid(
            row=1, column=1, columnspan=2, padx=5, pady=5, sticky="nsew"
        )

        insert_item_dialog.game_position_label.grid(
            row=2, column=0, padx=5, pady=5, sticky="nsew"
        )

        insert_item_dialog.game_position_spinbox.grid(
    ...
    def swap_items_ui(self):
        ...
        swap_items_dialog.title_label = tk.Label(
            swap_items_dialog, text="Swap Items", font=("Arial", 12)
        )
        ...
        swap_items_dialog.first_item = ttk.Combobox(
            swap_items_dialog,
            values=[
                self.wishlist_display.item(item)["values"][0]
                for item in self.wishlist_display.get_children()
            ],
        )

        swap_items_dialog.second_item = ttk.Combobox(
            swap_items_dialog,
            values=[
                self.wishlist_display.item(item)["values"][0]
                for item in self.wishlist_display.get_children()
        ...
        cancel_button = ttk.Button(
            swap_items_dialog, text="Cancel", command=swap_items_dialog.destroy
        )

        # Add the widgets to the dialog

        swap_items_dialog.title_label.grid(
            row=0, column=0, columnspan=2, padx=5, pady=5, sticky="nsew"
        )

        swap_items_dialog.item1_label.grid(
            row=1, column=0, padx=5, pady=5, sticky="nsew"
        )

        swap_items_dialog.item2_label.grid(
            row=1, column=1, padx=5, pady=5, sticky="nsew"
        )

        swap_items_dialog.first_item.grid(
            row=2, column=0, padx=5, pady=5, sticky="nsew"
        )

        swap_items_dialog.second_item.grid(
    ...
    def swap_items_in_database(self, name1: str, name2: str):
        ...
        self.database[index1], self.database[index2] = (
            self.database[index2],
            self.database[index1],
        )
    ...
    def change_database(self, action: str, game_id: int, data):
        ...
        if action == "add":
            ...
            if name1 is None or name2 is None:
                raise ValueError(
                    "Invalid data parameter. 'name1' and 'name2' keys are required."
                )
    ...
    def update_treeview(self):
        ...
        for i, data in enumerate(self.database):
            ...
            self.wishlist_display.insert(
                ...
                iid=i,
                text=data["game_id"],
                values=(data["name"], f"${price_data:.2f}", data["url"]),
            )
        ...
        print(
            f"The strings!: {self.mandatory_string_list}\n The floats!: {self.mandatory_float_list}"
        )
